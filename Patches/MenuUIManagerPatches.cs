...
namespace CivilizameMP.Patches
{
    ...
    public class MenuUIManagerPatches
    {
        ...
        [HarmonyPatch("Update")]
        [HarmonyPostfix]
        public static void UpdatePostfix(MenuUIManager __instance)
        {
            if (_mpButton == null) return;

            var startButton = FindStartButton(__instance.transform);
            bool startActive = startButton != null && startButton.gameObject.activeInHierarchy;

            if (_mpButton.activeSelf != startActive)
        ...
        private static void InjectMultiplayerButton(MenuUIManager menuUI)
        {
            ...
            try
            {
                ...
                _mpButton.SetActive(startButton.gameObject.activeInHierarchy);
